...
load("11102021_monocyte paper analysis_V5.RData")
...
#Plotting DEGs
ComplexHeatmap::pheatmap(batch.rem[rownames(batch.rem) %in% top100.up,], 
...
ComplexHeatmap::pheatmap(batch.rem[rownames(batch.rem) %in% top100.down,], 
                         cluster_rows = T, cluster_cols = T, annotation_legend = T, show_colnames = F, show_rownames = F,
                         scale='row',
                         legend = T, treeheight_row = 0,
                         annotation_col=annot_df, annotation_colors = annot_colors, fontsize_row = 8)
...
for (i in c("NFKB", "IFN", "Microglia")){
  panel <- panel.list[[i]]
  DE <- DE.list[[i]]
  df <- panel[DE$log2FoldChange < -1 | DE$log2FoldChange > 1,]
...
(2A) Scatter plot of signature with DEG co-variance
